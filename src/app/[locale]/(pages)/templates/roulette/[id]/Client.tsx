          ...
          if (templateData && templateData.is_template) {
            ...
            if (typeof currentDescription === "string") {
              ...
              if (i18n.language in currentDescription) {
                ...
                setDescription(
                  (Object.values(currentDescription)[0] as string) || ""
                );
              }
            }

            // テンプレート取得後、作成者のプロフィールを取得
            if (templateData.user_id && templateData.is_profile_public) {
              // ★ is_profile_publicがtrueの場合のみ取得
    ...
    <>
        ...
        <div>
          ...
          <div className="mt-4 flex justify-center">
            <button
              onClick={handleLike}
              disabled={isLiking}
              className="flex items-center gap-2 px-4 py-2 bg-white/20 hover:bg-white/30 rounded-lg text-white font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Heart
                size={18}
      ...
      {creatorProfile && isProfilePublic && (
        ...
        >
          ...
          <Link
            href={`/${i18n.language}/profiles/${creatorProfile.id}`}
