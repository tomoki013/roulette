...
const TemplateCard = ({ template }: TemplateCardProps) => {
  ...
  const truncatedTitle =
    template.title.length > 15
      ? `${template.title.substring(0, 15)}...`
      : template.title;
  const truncatedDescription =
    getDescription().length > 70
      ? `${getDescription().substring(0, 70)}...`
      : getDescription();
      ...
      >
        ...
        <div className="mt-4">
          <div className="flex justify-between items-center text-sm text-white/60 mb-4">
            {authorInfo ? (
              <p
                className={`flex items-center gap-2 transition-colors ${authorInfo.isOfficial ? "text-yellow-400" : "hover:text-yellow-300"}`}
              >
                <authorInfo.Icon
                  size={14}
                  className={authorInfo.isOfficial ? "fill-yellow-400" : ""}
                />
