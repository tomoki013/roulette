  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <motion.div
            className="bg-gray-800 rounded-2xl p-6 max-w-md w-full text-white border border-white/20 max-h-[70vh] overflow-y-auto"
          ...
          >
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between bg-white/10 p-4 rounded-lg">
                ...
                <button
                  ...
                  className={`w-[48px] h-[24px] rounded-full flex items-center transition-colors ${isTemplate ? "bg-yellow-400" : "bg-gray-600"}`}
                >
                  <motion.div
                    layout
                    className={`w-[20px] h-[20px] bg-white rounded-full shadow-md transform ${isTemplate ? "translate-x-[24px]" : "translate-x-[4px]"}`}
                  />
                </button>
              </div>
                ...
                {isTemplate && (
                  ...
                  >
                    ...
                    <button
                      ...
                      className={`w-[48px] h-[24px] rounded-full flex items-center transition-colors ${allowFork ? "bg-yellow-400" : "bg-gray-600"}`}
                    >
                      <motion.div
                        layout
                        className={`w-[20px] h-[20px] bg-white rounded-full shadow-md transform ${allowFork ? "translate-x-[24px]" : "translate-x-[4px]"}`}
                      />
                    </button>
                  </motion.div>
                )}
                ...
                {isTemplate && (
                  ...
                  >
                    ...
                    <button
                      ...
                      className={`w-[48px] h-[24px] rounded-full flex items-center transition-colors ${isProfilePublic ? "bg-yellow-400" : "bg-gray-600"}`}
                    >
                      <motion.div
                        layout
                        className={`w-[20px] h-[20px] bg-white rounded-full shadow-md transform ${isProfilePublic ? "translate-x-[24px]" : "translate-x-[4px]"}`}
                      />
                    </button>
                  </motion.div>
                )}
            ...
            <div className="mt-6 flex justify-end gap-4">
              ...
              <button
                onClick={handleSave}
                disabled={isSaving}
                className="px-6 py-2 text-sm font-semibold bg-yellow-500 hover:bg-yellow-600 rounded-lg transition-colors flex items-center gap-2 disabled:opacity-50"
              >
                {isSaving ? (
